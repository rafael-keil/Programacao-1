...
Modele uma solução na qual as principais classes são **jogador.Jogador** e **clube.Clube**. O detalhamento de cada uma delas está específicado ao longo desta seção.
...
### jogador.Jogador
O jogador.Jogador deve possuir atributos como: **nome**, **idade**, **clube atual**, **reputacao historica**, **apetite financeiro**, **preco**. Nenhum objeto pode ser do tipo jogador. Ao instaciar um jogador, o objeto deve ser de um dos tipos de especialização detalhados a seguir na sessão **Tipos de jogador.Jogador**.
...
RN24: o unico método público da classe negociação deve ser possuir seguinte assinatura `public boolean negociar(clube.Clube clubeInteressado, jogador.Jogador jogadorDeInteresse)`.
...
public void deveSerPossivelNegociarUmGoleiroComUmClubeQueTemSaldoEmCaixa() {
    ...
    clube.Clube clube = new clube.Clube("Grêmio", 10, BigDecimal.valueOf(100000000));
...
public void naoDeveSerPossivelNegociarUmAtacanteComUmClubeQueTemReputacaoHistoricaMenorQueASua() {
    ...
    Negociacao negociacao = new Negociacao();
    clube.Clube clube = new clube.Clube("Internacional", 3, BigDecimal.valueOf(100000000));
